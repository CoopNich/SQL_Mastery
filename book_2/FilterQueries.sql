...
SELECT s.vehicle_id,
    ...
    s.payment_method
FROM sales s
WHERE s.sales_type_id = 2 -- Get a list of sales 
...
SELECT s.vehicle_id,
    ...
    s.payment_method
FROM sales s
WHERE s.purchase_date >= now() - interval '2 years' -- Get a list of sales 
...
WHERE s.deposit >= 5000
    OR s.payment_method = 'americanexpress' -- Get a list of employees whose first names start with "M" 
    -- or ends with "E"
SELECT e.first_name,
    e.last_name
FROM employees e
WHERE e.first_name LIKE 'M%'
    OR e.first_name LIKE '%e' -- Get a list of employees 
